...
trait Exp {
  ...
  override def toString = this match {
    ...
    case While(cond, body) => s"${space}while ($cond) {\n$body\n$space}"
...
case class Update (x: UVar, e1: BExp, e: CExp) extends CExp // user definition 
case class If(b: AExp, then: CExp, els: CExp) extends CExp // branch
case class While(cond: AExp, body: CExp) extends CExp // while
