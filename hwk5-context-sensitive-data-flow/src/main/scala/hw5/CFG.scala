...
trait CFG {
  ...
  // retrieve the successors and predecessors of a hw5.CFG node
  def succ(n: Node) : Set[Node]
  def pred(n: Node) : Set[Node]
  
  def bypass(n: Node) : Set[Node] // returns the hw5.RetNode of a hw5.CallNode (forward) or the callNode of a hw5.RetNode (backward)
  ...
  private val imap = Map[Statement, IntraNode]()                    // intra-procedural hw5.CFG nodes
  ...
  // collect the hw5.CFG statements from a program
